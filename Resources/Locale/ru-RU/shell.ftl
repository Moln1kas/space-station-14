...
shell-server-cannot = Сервер не может сделать такое!
shell-command-success = Команда была успешно выполнена!
shell-invalid-command = Неправильная команда!
shell-invalid-command-specific = Неправильная {$commandName} команда!
shell-cannot-run-command-from-server = Вы не можете запустить данную команду на этом сервере!
shell-only-players-can-run-this-command = Только игроки могут запустить данную команду!
shell-must-be-attached-to-entity = Ты должен указать какую-либо сущность для запуска этой команды!
